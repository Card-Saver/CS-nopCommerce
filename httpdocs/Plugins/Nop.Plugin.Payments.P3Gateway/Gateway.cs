...
namespace Nop.Plugin.Payments.P3Gateway
{
    public class Gateway
    {
        ...
        private readonly string directUrl;
            ...
            Dictionary<string, string> options = null)
        {
            ...
            if (!string.IsNullOrEmpty(secret)) requestFlattened["signature"] = Sign(requestFlattened, secret);
            ...
            Dictionary<string, string> options = null)
        {
            ...
            if (!request.ContainsKey("redirectURL"))
                ...
                request["redirectURL"] = "https://www.example.net";

            if (!string.IsNullOrEmpty(secret)) request["signature"] = Sign(NestedDictionaryAdapter(request), secret);
            ...
            ret.Append($@"<form id=""silentPost"" method=""post"" {formAttrs} action=""{action}"" />");

            foreach (var name in request.Keys) ret.Append(FieldToHtml(name, request[name]));
            ...
            if (options.ContainsKey("submitImage"))
                ...
                submitElement = $"<input {submitAttrs} type=\"submit\" value=\""
                                + (options.ContainsKey("submitText")
                                    ? WebUtility.HtmlEncode(options["submitText"])
                                    : "Pay Now")
                                + "\" />\n";
        ...
        ///             <param name="secret">
        ///                 The Merchant Secret </params>
        ///                 <param name="directUrl">
        ///                     The URL for direct integrations </params>
        ...
        {
            if (request == null || request.Count == 0) throw new ArgumentException("Request must be provided.");

            if (!request.ContainsKey("action")) throw new ArgumentException("Request must contain an 'action'");
            ...
            foreach (var key in removeKeys) request.Remove(key); // Doesn't error if key not present. 
        }
        ...
        private static string GetUrlEncodedBody(IEnumerable<KeyValuePair<string, string>> fields)
        {
            var rtn = string.Join("&",
                fields.Select(f =>
                    string.Format("{0}={1}", WebUtility.UrlEncode(f.Key), WebUtility.UrlEncode(f.Value))));
        ...
        ///     Dictionary
        ///     <string, string>
        ///         containing the fields to be signed. </params>
        ///         <param name="secret">
        ///             secret used to create the signature </params>
        ///             <returns>
        ...
        public string Sign(Dictionary<string, string> fields, string secret = null, IEnumerable<string> partial = null)
        {
            ...
            if (secret == null && merchantSecret != null) secret = merchantSecret;

            // HttpUtility returns UPPERCASE percent encoded characeters
            var encodedFields = fields.OrderBy(
                f => f.Key.Contains("[") ? f.Key.Replace("[", "0").Substring(0, f.Key.IndexOf("[")) : f.Key,
        ...
        public string CollectBrowserInfo(Dictionary<string, object> options, Dictionary<string, string> env)
        {
            ...
            var deviceData = new Dictionary<string, string>
            {
                ...
                {
                    "deviceIdentity",
                    env.ContainsKey("HTTP_USER_AGENT") ? WebUtility.HtmlEncode(env["HTTP_USER_AGENT"]) : null
                },
                {"deviceTimeZone", "0"},
                {"deviceCapabilities", ""},
                ...
                {
                    "deviceAcceptContent",
                    env.ContainsKey("HTTP_ACCEPT") ? WebUtility.HtmlEncode(env["HTTP_ACCEPT"]) : null
                },
                {
                    "deviceAcceptEncoding",
                    env.ContainsKey("HTTP_ACCEPT_ENCODING") ? WebUtility.HtmlEncode(env["HTTP_ACCEPT_ENCODING"]) : null
                },
                {
                    "deviceAcceptLanguage",
                    env.ContainsKey("HTTP_ACCEPT_LANGUAGE") ? WebUtility.HtmlEncode(env["HTTP_ACCEPT_LANGUAGE"]) : null
                }
            };
            ...
            if (options.ContainsKey("formData"))
            {
                ...
                foreach (var name in formData.Keys) formFieldsSb.Append(FieldToHtml(name, formData[name]));
            }
        ...
        ///     Dictionary
        ///     <string, string>
        ///         containing the fields the server has responded with. </params>
        ///         <param name="secret">
        ///             secret used to create the signature </params>
        ///             <returns>
        ...
        {
            if (response == null || response.Count == 0) throw new ArgumentException("Invalid response from Gateway");

            if (string.IsNullOrEmpty(secret)) secret = merchantSecret;
            ...
            if (!string.IsNullOrEmpty(secret) && Sign(response, secret, fields) != signature)
                // Signature mismatch
        ...
        public static string FieldToHtml(string name, Dictionary<string, string> value)
        {
            ...
            foreach (var k in value.Keys) sb.Append(FieldToHtml(name + $"[{k}]", value[k]));
        ...
        private static Dictionary<string, object> NestedDictionaryAdapter(Dictionary<string, string> inDict)
        {
            ...
            foreach (var k in inDict.Keys)
                if (k.Contains("[") && k.IndexOf("]") > k.IndexOf("["))
                {
                    ...
                    if (!rtn.ContainsKey(outerKey)) rtn[outerKey] = new Dictionary<string, string>();
        ...
        private static Dictionary<string, string> NestedDictionaryAdapter(Dictionary<string, object> inDict)
        {
            ...
            foreach (var k in inDict.Keys)
                ...
                else if (inDict[k].GetType() == typeof(Dictionary<string, string>))
                {
                    ...
                    foreach (var nestedK in nestedD.Keys) rtn[$"{k}[{nestedK}"] = nestedD[nestedK];
                }
